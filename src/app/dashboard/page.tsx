...
export default async function DashboardPage() {
  ...
  // Le pedimos a Supabase las pantallas (Globales por ahora, o asignadas)
  ...
  return (
    <div className="p-8 min-h-screen bg-zinc-50 font-[family-name:var(--font-geist-sans)]">
      ...
      <div className="mb-12">
        <h2 className="text-xl font-bold text-zinc-800 mb-4">Mis Pantallas Disponibles</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {pantallas && pantallas.length > 0 ? (
            ...
            <div className="col-span-full py-12 text-center bg-white rounded-xl border border-dashed border-zinc-300">
              <p className="text-zinc-500 italic">No se encontraron pantallas en tu red.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div>
        <h2 className="text-xl font-bold text-zinc-800 mb-4">Mis Campañas</h2>
        <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
          {misCampanas && misCampanas.length > 0 ? (
                  ...
                  <tr key={camp.id} className="hover:bg-zinc-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${
                        ...
                        camp.estado === 'rechazada' ? 'bg-red-100 text-red-700' :
            ...
            <div className="p-12 text-center">
              <p className="text-zinc-500 italic mb-4">No tienes ninguna campaña creada todavía.</p>
              <Link href="/dashboard/nueva">
                <Button variant="outline">Crear mi primera campaña</Button>
              </Link>
            </div>
          )}
        </div>
